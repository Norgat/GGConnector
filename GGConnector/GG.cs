...
namespace GGConnector {
    public class GG: IDisposable {
        ...
        private bool welcome_flag = false;
        private Welcome _welcome = null;
        public async Task<Welcome> Connect() {
            if (_socket != null) {
                _socket.Close();
            }
            ...
            OnGetWelcome += GG_OnGetWelcome;

            while (welcome_flag == false) {
                Thread.Sleep(1);
            }
        ...
        private void GG_OnGetWelcome(object sender, Welcome welcome) {
            welcome_flag = true;
            _welcome = welcome;
        }
